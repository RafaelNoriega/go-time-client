...
exports.asyncCreateFileBody = (rows, type) => {
    ...
    return new Promise( (resolve) => {
                        ...
                        if(breaksAggregate.has(id)){
                            ...
                            for(let [index, set] of obj.set1.entries()){
                                if(parseFloat(set.rate1) == parseFloat(rate1)){
                                    set1Found = true;
                                    obj.set1[index].pieces1 += parseInt(pieces1);   
                                }
                            }
                            ...
                            for(let [index, set] of obj.set2.entries()){
                                if(parseFloat(set.rate2) == parseFloat(rate2)){
                                    set2Found = true;
                                    obj.set1[index].pieces2 += parseInt(pieces2);   
                                }
                            }
                            ...
                            for(let [index, set] of obj.set3.entries()){
                                if(parseFloat(set.rate3) == parseFloat(rate3)){
                                    set3Found = true;
                                    obj.set3[index].pieces3 += parseInt(pieces3);   
                                }
                            }
        ...
        for(const [key, value] of breaksAggregate){
            const {hours, breakTime, nonProductiveTimeSet, job, date, costCenter, state, set1, set2, set3} = value;
            let rate = 0.00;
            ...
            //state 0 = piece Only, 1 = piece and hourly
            if(state === 0){
                rate = parseFloat(pieceCompensation / (parseFloat(hours) - parseFloat(nonProductiveTimeInHours) - parseFloat(breakTimeInHours)) );
                ...
                if(overTimeFlag){
                    ...
                    let totalCompensation = parseFloat(pieceCompensation + ( adjustedWorkHours * process.env.MINIMUM_WAGE) );
                    rate = parseFloat(totalCompensation / (parseFloat(hours) - parseFloat(nonProductiveTimeInHours) - parseFloat(breakTimeInHours)));
                }
            }    
            ...
            for(let set of set1){
                if(parseInt(set.pieces1) > 0 && parseFloat(set.rate1) > -1 ){
                    console.log(set.pieces1, set.rate1);
                    if(state === 1){
                        ...
                        // id = '', date = '', costCenter = '', hours = '', pieces = 1, rate = 1, taskName = '', payCode= '  ', tcRate = '')
                    }
                }
            }
            for(let set of set2){
            ...
            fileBody +=  createFileRow(key , nonProductiveTimeSet.date, costCenter, nonProductiveTimeSet.time/60.00, 0, 0, 'Non Productive Time', 'HR', 14, type); 
