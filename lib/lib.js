...
exports.asyncCreateFileBody = (rows = [], type = 'summary', crewId = 0000) => {
    ...
    function createFileRowAgStar(id = '', date = '', costCenter = '', hours = '', pieces = 1, rate = 1, taskName = '', payCode = ' ', tcRate = ''){
        ...
        fileBody += moment(date, 'YYYY MM DD').format('MM/DD/YYYY');
        //unused
        fileBody += '      ';
        //*Cost center code
    ...
    return new Promise( (resolve) => {
        ...
        //Find if employees have any weekly OT before writing rows to the export
        for(const row of rows){
            const {id, date, breakTime, nonProductiveTime} = row;
            let hours = row.hours;
            if(parseFloat(hours) > parseFloat(process.env.OVERTIME_DAY)){
                hours -= parseFloat(hours) - parseFloat(process.env.OVERTIME_DAY);
            }

            if(trackWeeklyOT.has(id)){
                let obj = trackWeeklyOT.get(id);
                obj.TotalHoursWeekly += parseFloat(hours);
                obj.daysWorked.push(date);

                obj.daysWorked.sort();
                if(parseFloat(obj.TotalHoursWeekly) > parseFloat(process.env.OVERTIME_WEEK)){
                    obj.weeklyOT = true;
                    obj.OtTotal = parseFloat(obj.TotalHoursWeekly) - parseFloat(process.env.OVERTIME_WEEK);
                    obj.daysToSubtractOT.push({date, OtHours: parseFloat(obj.OtTotal), hoursWorked:hours, breakTime, nonProductiveTime});
                    obj.TotalHoursWeekly -= parseFloat(obj.OtTotal);
                }
                ...
                let obj = {
                    ...
                    daysWorked: [date],
                    OtTotal: 0.00,
                    TotalHoursWeekly: parseFloat(hours),
                    daysToSubtractOT: []
                }

                trackWeeklyOT.set(id, obj);
            }
        }
        ...
        for(const row of rows){
            ...
            if(parsefloat(row.wage) != 0 ||row.wage.length > 0 || row.wage !== undefined){
                wage = row.wage
            }
            ...
            if(exported == false || exported == 'false'){
                const record = trackWeeklyOT.get(id);
                const otHoursForDay = record.daysToSubtractOT.find( day => day.date == date);

                //Hourly Pay Only
                if(parseInt(pieces1) <= 0 && parseInt(pieces2) <= 0 && parseInt(pieces3) <= 0){
                    hourlyOnly = true;
                    if(otHoursForDay != undefined){
                        if(parseFloat(hours) > process.env.OVERTIME_DAY 
                        && (parseFloat(hours) - parseFloat(hours - process.env.OVERTIME_DAY) - parseFloat(otHoursForDay.OtHours)) > 0){
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours) - parseFloat(hours - process.env.OVERTIME_DAY) - parseFloat(otHoursForDay.OtHours), 0, 0, job, 'HR', parseFloat(wage), type);
                        }else if( ( parseFloat(hours) - parseFloat(otHoursForDay.OtHours) ) > 0){
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours) - parseFloat(otHoursForDay.OtHours), 0, 0, job, 'HR', parseFloat(wage), type);
                        }
                    }else{
                        if(parseFloat(hours) > process.env.OVERTIME_DAY){
                            ...
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours), 0, 0, job, 'HR', parseFloat(wage) , type);
                        }
                    }
                }
                ...
                if(parseFloat(hours) > process.env.OVERTIME_DAY){
                    if(overTime.has(id)){
                        ...
                        if(moment(date).format('MM/DD/YYYY') > obj.date){
                            obj.date = moment(date, 'YYYY MM DD').format('MM/DD/YYYY');
                        }
                        if(hourlyOnly){
                    ...
                    if(overTime.has(id)){
                        ...
                        if(moment(date, 'YYYY MM DD').format('MM/DD/YYYY') > obj.date){
                            obj.date = moment(date, 'YYYY MM DD').format('MM/DD/YYYY');
                        }
                        if(hourlyOnly){
                ...
                if( 
                  ( ( parseInt(pieces1) > 0 && parseFloat(rate1) > -1) 
                 || ( parseInt(pieces2) > 0 && parseFloat(rate2) > -1) 
                 || ( parseInt(pieces3) > 0 && parseFloat(rate3) > -1) )
                 && (pieceOnly == false || pieceOnly == 'false')){
                    if(otHoursForDay != undefined){
                        if(((parseFloat(hours) - parseFloat(otHoursForDay.OtHours)) > 0) ){
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours - ( parseFloat(breakTime) / 60.00) - parseFloat(nonProductiveTime/60) - parseFloat(otHoursForDay.OtHours) ), 0, 0, job, 'HR', parseFloat(wage), type);
                        }
                    }else{
                    ...
                    if(pieceOnly == 'true' || pieceOnly == true ){
                        ...
                        if(breaksAggregate.has(id)){
                            ...
                            if(parseInt(nonProductiveTime) > 0){
                                obj.nonProductiveTimeSet.date = moment(date, 'YYYY MM DD').format('MM/DD/YYYY')
                            ...
                            if(parseInt(nonProductiveTime) > 0){
                                nonProductiveTimeSet = {date: moment(date, 'YYYY MM DD').format('MM/DD/YYYY'), time: nonProductiveTime}
                            }
        ...
        for(const [key, value] of breaksAggregate){
            ...
            for(let set of set1){
                if(parseInt(set.pieces1) > 0 && parseFloat(set.rate1) > -1 ){
                    if(state === 1){
                        fileBody +=  createFileRow(key , moment(date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, 0 , set.pieces1, set.rate1, job, 'PC', '' , type); 
                    }else if(state === 0){
                        for(let pieceOnlyRow of pieceOnly1Rows){
                            fileBody +=  createFileRow(key , moment(pieceOnlyRow.date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, parseFloat(parseFloat(pieceOnlyRow.hours) - nonProductiveTimeInHours - breakTimeInHours) , pieceOnlyRow.pieces1, pieceOnlyRow.rate1, job, 'PC', '' , type); 
                        }
                    }
                }
            }
            for(let set of set2){
                if(parseInt(set.pieces2) > 0 && parseFloat(set.rate2) > -1 ){
                    if(state === 1){
                        fileBody +=  createFileRow(key , moment(date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, 0 , set.pieces2, set.rate2, job, 'PC', '', type);
                    }else if(state === 0){
                        for(let pieceOnlyRow of pieceOnly2Rows){
                            fileBody +=  createFileRow(key , moment(pieceOnlyRow.date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, parseFloat(parseFloat(pieceOnlyRow.hours) - nonProductiveTimeInHours - breakTimeInHours) , pieceOnlyRow.pieces2, pieceOnlyRow.rate2, job, 'PC', '' , type); 
                        }                    }                    
                }
            }
            for(let set of set3){
                if(parseInt(set.pieces3) > 0 && parseFloat(set.rate3) > -1 ){
                    if(state === 1){
                        fileBody +=  createFileRow(key , moment(date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, 0 , set.pieces3, set.rate3, job, 'PC', '', type);
                    }else if(state === 0){
                        for(let pieceOnlyRow of pieceOnly3Rows){
                            fileBody +=  createFileRow(key , moment(pieceOnlyRow.date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, parseFloat(parseFloat(pieceOnlyRow.hours) - nonProductiveTimeInHours - breakTimeInHours) , pieceOnlyRow.pieces3, pieceOnlyRow.rate3, job, 'PC', '' , type); 
                        }                    }                    
                }
            }
            
            if(parseFloat(nonProductiveTimeSet.time) > 0){
                fileBody +=  createFileRow(key , moment(date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, nonProductiveTimeSet.time/60.00, 0, 0, 'Non Productive Time', 'HR', parseFloat(wage), type); 
            }
            

            //Rest And Recovery
            fileBody +=  createFileRow(key , moment(date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, breakTimeInHours, '', '', 'Rest and Recovery', 'HR', rate, type)     
        }

        //Overtime
        for(const [key, value] of overTime){
            const {totalRegularHours, overTimeDaily, job, state, costCenter, date} = value
            const record = trackWeeklyOT.get(key);
            const otHoursForDay = record.daysToSubtractOT.find( day => day.date == date);
            ...
            for(const day of overTimeDaily){
                const {hours} = day;
                if(day.state == 1){
                    fileBody += createFileRow(key, moment(day.date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, hours, '', '', day.job, 'OV', '' , type);
                }
            }
            ...
            if(parseFloat(totalRegularHours) > process.env.OVERTIME_WEEK){
                //let totalOverTimeHours = parseFloat(totalRegularHours) - process.env.OVERTIME_WEEK;
                 //state 0 = piece Only, 1 = piece and hourly
                if(state == 1){
                    // fileBody += createFileRow(key, moment(value.date, 'MM DD YYYY').format('YYYY MM DD'), costCenter, totalOverTimeHours, '', '', job, 'OV', '', type);
                    record.daysToSubtractOT.forEach( function (entry) {
                        if(parseFloat(entry.OtHours) == parseFloat(entry.hoursWorked)){
                            fileBody += createFileRow(key, moment(entry.date, 'YYYY MM DD'), costCenter, entry.OtHours - (parseFloat(entry.nonProductiveTime)/60.00 ) - (parseFloat(entry.breakTime)/60.00 ), '', '', job, 'OV', '', type)
                        }else{
                            fileBody += createFileRow(key, moment(entry.date, 'YYYY MM DD'), costCenter, entry.OtHours, '', '', job, 'OV', '', type)
                        }
                    });
                }
            }
...
exports.fetchData = async (employeeId, costCenter, startDate, endDate, user) => {
    return new Promise(  async (resolve, reject) => {
        ...
        let data = await docClient.query(params).promise().catch(error => console.log(error));
        
        while (typeof data.LastEvaluatedKey != "undefined") {
            params.ExclusiveStartKey = data.LastEvaluatedKey;
    
            let newData = await docClient.query(params).promise().catch(error => console.log(error));
            data.Items.push(...newData.Items);
            data.LastEvaluatedKey = newData.LastEvaluatedKey;
        }
