        ...
        for(const row of rows){
            ...
            if(trackWeeklyOT.has(id)){
                ...
                if(parseFloat(obj.TotalHoursWeekly) > parseFloat(process.env.OVERTIME_WEEK)){
                    ...
                    obj.daysToSubtractOT.push({date, hours: parseFloat(obj.OtTotal)});
            ...
            if(exported == false || exported == 'false'){
                ...
                if(parseInt(pieces1) <= 0 && parseInt(pieces2) <= 0 && parseInt(pieces3) <= 0){
                    ...
                    if(otHoursForDay != undefined){
                        ...
                        && (parseFloat(hours) - parseFloat(hours - process.env.OVERTIME_DAY) - parseFloat(otHoursForDay.hours)) > 0){
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours) - parseFloat(hours - process.env.OVERTIME_DAY) - parseFloat(otHoursForDay.hours), 0, 0, job, 'HR', parseFloat(wage), type);
                        }else if( ( parseFloat(hours) - parseFloat(otHoursForDay.hours) ) > 0){
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours) - parseFloat(otHoursForDay.hours), 0, 0, job, 'HR', parseFloat(wage), type);
                        }
                    }else{
                        if(parseFloat(hours) > process.env.OVERTIME_DAY){
                ...
                if( 
                 ...
                 && (pieceOnly == false || pieceOnly == 'false')){
                    if(otHoursForDay != undefined){
                        if(((parseFloat(hours) - parseFloat(otHoursForDay.hours)) > 0) ){
                            fileBody +=  createFileRow(id, date, costCenter, parseFloat(hours - ( parseFloat(breakTime) / 60.00) - parseFloat(nonProductiveTime/60) - parseFloat(otHoursForDay.hours) ), 0, 0, job, 'HR', parseFloat(wage), type);
                        }
                    }else{
        ...
        for(const [key, value] of overTime){
            ...
            if(parseFloat(totalRegularHours) > process.env.OVERTIME_WEEK){
                ...
                if(state == 1){
                    ...
                    record.daysToSubtractOT.forEach( entry => fileBody += createFileRow(key, moment(entry.date, 'YYYY MM DD'), costCenter, entry.hours, '', '', job, 'OV', '', type));
                }
            }
        }
