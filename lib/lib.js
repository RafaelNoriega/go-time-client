    ...
    return new Promise( (resolve) => {
        ...
        for(const [key, value] of breaksAggregate){
            ...
            if(state === 0){
                ...
                if(overTimeFlag){
                    ...
                    if(parseFloat(wage) > 0.00){
                        ...
                        totalNormalHoursCompensation = parseFloat( parseFloat(adjustedWorkHours) * process.env.MINIMUM_WAGE);
                        totalOverTimeHoursCompensation = parseFloat(totalOverTimeHours) * (process.env.MINIMUM_WAGE * 1.5);   
                    }
                    ...
                    if(parseFloat(wage) > 0.00){
                        ...
                        hoursCompensation = parseFloat( adjustedWorkHours * process.env.MINIMUM_WAGE);
                    }
                    //console.log('hours compensation: ', hoursCompensation);
                    //console.log('piece compensation: ', pieceCompensation);
                ...
                if(parseInt(set.pieces1) > 0 && parseFloat(set.rate1) > -1 ){
                    if(state === 1){
                        ...
                        for(let pieceOnlyRow of pieceOnly1Rows){
                            fileBody +=  createFileRow(key , pieceOnlyRow.date, costCenter, parseFloat(hours - nonProductiveTimeInHours - breakTimeInHours) , pieceOnlyRow.pieces1, pieceOnlyRow.rate1, job, 'PC', '' , type); 
                        }
                    }
                }
            }
                ...
                if(parseInt(set.pieces2) > 0 && parseFloat(set.rate2) > -1 ){
                    if(state === 1){
                        ...
                        for(let pieceOnlyRow of pieceOnly2Rows){
                            fileBody +=  createFileRow(key , pieceOnlyRow.date, costCenter, parseFloat(hours - nonProductiveTimeInHours - breakTimeInHours) , pieceOnlyRow.pieces2, pieceOnlyRow.rate2, job, 'PC', '' , type); 
                        }                    }                    
                }
            }
            for(let set of set3){
                if(parseInt(set.pieces3) > 0 && parseFloat(set.rate3) > -1 ){
                    if(state === 1){
                        ...
                        for(let pieceOnlyRow of pieceOnly3Rows){
                            fileBody +=  createFileRow(key , pieceOnlyRow.date, costCenter, parseFloat(hours - nonProductiveTimeInHours - breakTimeInHours) , pieceOnlyRow.pieces3, pieceOnlyRow.rate3, job, 'PC', '' , type); 
                        }                    }                    
                }
            }
            ...
            if(parseFloat(wage) > 0.00){
                ...
                fileBody +=  createFileRow(key , date, costCenter, nonProductiveTimeSet.time/60.00, 0, 0, 'Non Productive Time', 'HR', 14, type); 
            }
